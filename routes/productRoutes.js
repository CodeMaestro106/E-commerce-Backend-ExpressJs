...
router.route('/')
    .post(authenticate, authorize([config.roles.admin]), upload.single('productImage'), createProduct);

router.route('/:id')
    .put(authenticate, authorize([config.roles.admin]), upload.single('productImage'), updateProduct)
